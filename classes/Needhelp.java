...
public class Needhelp implements ActionListener{
	
	JFrame Frame;
	
	private JTextArea n;
	private JTextField searchf;
	private JLabel background,pageName,lbl,lbl1;
	private JButton ad, backbtn, exitbtn,searchlbl,moviebtn,seriesbtn,reqbtn,popbtn,s;
	...
	public Needhelp()
	{
		...
		//ADD ADMIN LABEL
		lbl = new JLabel("CONTACT SUPPORT");
		...
		//ADD ADMIN LABEL
		lbl1 = new JLabel("Write Your Problems to Us");
		...
		if(e.getSource()==ad)
		{
			...
			if(help.isEmpty()==true)
			{
				...
				try{
					...
					File hfile = new File("data\\admin\\support_mail.txt");
					...
					PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(hfile, true)));
					
					pw.println("Username: "+Login.uprofile);
					pw.println("Message: "+help);
					pw.println("================================");
					pw.close();
				...
				showMessageDialog(null, "Thanks for contacting us.", "Sent!", 1);
